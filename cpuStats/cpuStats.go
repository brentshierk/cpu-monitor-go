...
func GetCoresStats() *CPUStat {
	...
	*/
	...
	for _, statLine := range dataSlice {
		...
		if len(statSlice) > 0 {
			if statSlice[0] != "" && statSlice[0][:3] == "cpuStats" {
...
func ComputeCoreLoad(currentStat, previousStat *CoreStat) float32 {
	...
	/*
	   ...
	   cpuStats  74608   2520   24433   1117073   6176   4054  0        0      0      0
	   *
	   *    Idle = idle + iowait
...
func ExtractCPUFrequency() *CPUFreq {
	...
	*/
	...
	var cpuFreq CPUFreq
	cpuInfoBytes, err := ioutil.ReadFile("/sys/devices/system/cpuStats/cpu0/cpufreq/scaling_cur_freq")
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
...
func ExtractCPUInfoFrequency() {
	...
	*/
	...
	for _, infoLine := range dataSlice {
		...
		if len(info) > 1 && info[0] != "" && info[0] == "cpuStats" && info[1][:3] == "MHz" {
			fmt.Println(info[2])
		}
	}
